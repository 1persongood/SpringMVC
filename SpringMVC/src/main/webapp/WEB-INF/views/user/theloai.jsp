...
<body>
	<div class="row">
		<div class="col-md-3" style="border-radius: 26px; border: 1px solid #dea3a3; background-image: linear-gradient(to top, #a8edea 0%, #fed6e3 100%);">
			<p
				...
				<i aria-hidden="true"></i><font
					style="color: #087BB0; text-align: center"> DANH MỤC SÁCH</font>
			</p>
		...
		<div class="col-md-9">
			<!-- @*sách theo chủ đề*@ -->
			<c:forEach var="item" items="${tentheloai}" begin="0" end="0">
			<h3 style="color: #087BB0">${item.getTenLoai()}</h3>
			</c:forEach>
			<hr />
			<c:if test="${BookPaginate.size()>0}">
						
			<div class="row">
				<c:forEach var="item" items="${BookPaginate}" varStatus="loop">
